...
DB_NAME = "final_project.db"   # the one shared DB me and Noah use
...
def create_tables():
    ...
    # Many rows per city (weather observations)
    cur.execute("""
...
def get_locations_needing_weather(limit=25):
    ...
    conn.close()
    return locations  # list of (id, lat, lon)
...
def store_weather_row(location_id, weather_json):
    ...
    """
    # /data/2.5/weather puts the current time in "dt"
    timestamp = weather_json["dt"]                 # Unix time
    ...
    cloud_cover = clouds.get("all", None)          # percent cloudiness
